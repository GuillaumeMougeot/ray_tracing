...
void PNGTest()
{
  ...
  Vector3d s1_pos(3,0,-1);
  Vector3d s1_center(s1_pos);
  double s1_radius(2);

  Vector3d s1_ka(0.5,0,0);
  Vector3d s1_kd(0.7,0,0);
  ...
  // Sphere 3
  Quaterniond s3_rot(1,0,0,0);
  Vector3d s3_pos(3,-1,2);
  Vector3d s3_center(s3_pos);
  double s3_radius(0.5);

  Vector3d s3_ka(0,0.5,0);
  Vector3d s3_kd(0,0,0.6);
  Vector3d s3_ks(0.5,0.5,0.5);
  double s3_n(30);
  Material s3_mat(s3_ka, s3_kd, s3_ks, s3_n);

  Sphere s3(s3_pos, s3_rot, s3_center, s3_radius, s3_mat);

  // Sphere 4
  Quaterniond s4_rot(1,0,0,0);
  Vector3d s4_pos(2,1,2);
  Vector3d s4_center(s4_pos);
  double s4_radius(1);

  Vector3d s4_ka(0.2,0.2,0.2);
  Vector3d s4_kd(0,0,0.6);
  Vector3d s4_ks(0.5,0.5,0.5);
  double s4_n(30);
  Material s4_mat(s4_ka, s4_kd, s4_ks, s4_n);

  Sphere s4(s4_pos, s4_rot, s4_center, s4_radius, s4_mat);

  // Triangle definition
  Quaterniond t1_rot(1,0,0,0);
  Vector3d t1_pos(0,-2,2);
  ...
  Triangle t1(t1_pos, t1_rot, t1_pos+ t1_v1, t1_pos+t1_v2, t1_pos+t1_v3, t1_mat);
  ...
  scene.AddPhysicalObject(&s3);
  scene.AddPhysicalObject(&s4);
  ...
  Camera cam(cam_pos, cam_rot, 800, 600, 0, 20, 0.01);
